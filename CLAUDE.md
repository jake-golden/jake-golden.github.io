...
| `lightbox.js` | One shared image+video lightbox (was copy-pasted into 5 pages). White background by default (many engineering-page images are transparent PNGs — schematics/diagrams — that read as illegible on black), overridable per-image via an opt-in `data-lightbox-bg` attribute on the trigger element (e.g. `data-lightbox-bg="#000"`, used by `ni.html`'s DIBNITS diagrams, which are dark-canvas graphics) — omitted entirely, the CSS default applies, so this is a no-op for every existing caller. Triggers: `.clickable-lightbox` elements (any tag, incl. `<a>` — click is `preventDefault`ed), images inside `.media-carousel`, and `[data-lightbox-video]` elements. The close button (`.lightbox-close`, shared CSS) sits just outside the image's top-left corner (`top:0; left:-1.75rem`, its own top edge flush with the image's) rather than inset over it, so it doesn't cover top-left image content (titles/labels) — falls back to a small inset position below 600px where there's no room outside. |
...
| `ni1.html`, `ni2.html` | **Two competing layout drafts of one unpublished page** (National Instruments internship), superseded by the live `ni.html` placeholder below — kept around as reference/scratch, not wired into `projects-data.js`. `ni1.html` also defines a page-local `.img-ph` placeholder class — deliberately *not* named `.img-placeholder`, because `cpu.html` already has a same-named class with hardcoded light-mode colors that predates the theme system. |

**`ni.html` is live** (not unlinked) and, as of this phase, substantially built out — no longer the bare "coming soon" placeholder. `projects-data.js`'s record (`slug: 'ni'`) still has `draft: true` and an empty `thumbnail` (keeps it off the engineering grid) — that's the only thing left before it can be published/linked; unlike the earlier plan, content was written fresh for `ni.html` rather than ported from `ni1`/`ni2` (those stay untouched scratch drafts, not a source of truth for this page). Current structure, top to bottom:
- **Goal card**: intro paragraph + a `Background Info` icon-row list, a YouTube `<iframe>` embed (`.ratio.ratio-16x9`), and the `signal_flow_path-light`/`-dark` diagram (the dark/light image-swap system above, `data-light-src`/`-dark-src`) — stacked in a `col-lg-6`/`col-lg-6` split (lg, not the usual md, since this page's bullets need the extra width before wrapping).
- **Evaluation Questions card**: a 6-row icon-tile list.
- Both the Goal card's bullets and Evaluation Questions reuse one page-local component, **`.eval-list`/`.eval-row`/`.eval-icon`/`.eval-text`** (icon square + label row, zebra removed in favor of one uniform `--chip-bg` tint per a later revision — light mode nudges it to `#fafbfc` since `--chip-bg` read too blue/gray at full strength, and default `--surface` is literally the same white as `.content-box` there). Not yet promoted to `css/styles.css` as a shared component — currently only exists inline in `ni.html`.
- **Two `.expandable-card` tiles** ("Cable Parasitic Tests", "DIB Non-Ideality Tests"), each a 2-column `row` (bullets left, `.media-carousel` right) inside `.expandable-content`. Cable Parasitic Tests' 4 slides (`cable_parasitic_tests_1-4.webp/png`) all share the same 7:5 aspect ratio, so that carousel's `.carousel-inner` is fixed to `aspect-ratio:7/5` with each `<img>` at `width/height:100%; object-fit:cover` — deliberately no background ever shows, unlike every other `.media-carousel` on the site (which use `max-height` + `object-fit:contain` and can letterbox). DIB Non-Ideality Tests' 6 slides (`DIBNITS_1-6.webp/png`) are dark-canvas CAD-tool screenshots (semi-transparent black backgrounds, colorful traces) — deliberately given no white bezel, unlike `pacemaker.html`'s convention, since forcing one would fight their intended dark look; all 6 open in the lightbox with `data-lightbox-bg="#000"` (see `lightbox.js` above).
- **Gallery strip** (`js/gallery-strip.js`, same component as Stryker's) with 5 real photos (`ni_gallery_1-5.webp/png`), no lightbox (matching that component's sitewide convention of not being lightbox-enabled).
- Its own page-local logo-sizing CSS (NI's mark is a compact 1.53:1 glyph, not a wordmark, so the shared `.banner-logo` sizing needed per-page overrides).
